import {
  ...
  export default function Fruitscreen() {
    ...
    const { fruitName } = route.params as RouteParams;
    const fruitData = Fruits[fruitName];
  
    if (!fruitData) {
      return (
        <View>
          <Text>fruit data not found for {fruitName}</Text>
        </View>
      );
    }
      ...
      const handleClick = () => {
        ...
        if (!flipped) {
          const randomIndex = Math.floor(
            Math.random() * fruitData.ciekawostki.length
          );
            ...
            {!flipped ? ( //jesli karta jest odkryta
              ...
              <Pressable onPress={handleClick} style={[styles.funfact, {backgroundColor: "mediumpurple"}]}>
                ...
                {randomFactIndex !== null ? (
                  <Text style={styles.facttext}>{fruitData.ciekawostki[randomFactIndex]}</Text>
                ) : (
    ...
    const speakfruitName = () => {
      speak(fruitData.name, { language: "pl", _voiceIndex: 1 }); // Speak the fruit's name in Polish
    };
      ...
      <View style={styles.container}>
        ...
        <View style={styles.infoContainer}>
          <Image
            ...
            style={styles.backgroundImage}
          />
          <Pressable onPress={speakfruitName} style={styles.nameContainer}>
            <Text style={styles.name}>{fruitData.name}</Text>
