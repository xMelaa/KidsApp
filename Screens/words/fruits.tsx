...
export default function FruitsScreen({ navigation }: HomeScreenProps) {
  ...
  const renderPage = (page: number) => {
    ...
    return fruitsNames.slice(startIndex, endIndex);
  };
      ...
      <View style={styles.contentContainer}>
        ...
        <SafeAreaView style={styles.buttonsContainer}>
          <FlatList
            ...
            keyExtractor={(fruitName) => fruitName}
            numColumns={itemsPerRow}
            horizontal={false}
            renderItem={({ item: fruitName }) => (
              <TouchableOpacity
                onPress={() =>
                  navigation.push("fruit", { fruitName: fruitName })
                }
                style={styles.itemContainer}>
                <Image source={Fruits[fruitName].photo} style={styles.image} />
              </TouchableOpacity>
            )}
        ...
        <View style={styles.iconButton}>
          {currentPage <
            Math.ceil(fruitsNames.length / (itemsPerRow * rowsPerPage)) && (
            <TouchableOpacity
              onPress={() => setCurrentPage(currentPage + 1)}
              style={styles.iconContainer}>
              <Icon
